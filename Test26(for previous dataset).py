import keras
import pandas as pd
df=pd.read_csv(r"C:\Users\User\Desktop\PROJECT\Work\DataSets\d.csv",encoding="ISO-8859-1")

new_X=list(df['content'].values)
new_Y=list(df['sentiment'].values)
diction={'happiness':0, 'hate':1, 'love':2, 'worry':3, 'fun':4, 'relief':5,\
        'enthusiasm':6, 'boredom':7, \
        'sadness':8, 'neutral':9, 'anger':10, 'empty':11, 'surprise':12}

Y=list([diction[item] for item in new_Y])
print(Y[:5])
